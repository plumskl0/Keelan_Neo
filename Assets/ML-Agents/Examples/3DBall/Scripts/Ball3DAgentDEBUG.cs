...
public class Ball3DAgentDEBUG : Agent
{
    [Header("Specific to Ball3D")]
    public GameObject ball;
    public Text debugTextX;
    public Text debugTextZ;
    ...
    public override void AgentAction(float[] vectorAction, string textAction)
    {
        if (brain.brainParameters.vectorActionSpaceType == SpaceType.continuous)
        {
                ...
                (gameObject.transform.rotation.z > -0.25f && action_z < 0f))
            {
                ...
                debugTextZ.text = string.Format("({0} , {1})", action_z, gameObject.transform.eulerAngles.z);
            }
                ...
                (gameObject.transform.rotation.x > -0.25f && action_x < 0f))
            {
                ...
                debugTextX.text = string.Format("({0} , {1})", action_x, gameObject.transform.eulerAngles.x);
            }
